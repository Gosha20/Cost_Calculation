...
object Parser {
  ...
  val wordIntoBark: Parser[String] = char('"') ~> takeWhile1(c => c != '"') | endOfInput.map(_ => "somewhere")

  val buyShort: Parser[Command] = (stringCI("/buy ") ~> money).map(SimpleCommand.Buy( "somewhere", "some", _))

  val buyDetail: Parser[Command] = (stringCI("/buy ") ~> wordIntoBark <~ string("\""),
    whitespaces ~> word <~ whitespaces,
    money).mapN(SimpleCommand.Buy.apply)

  val buyMedium: Parser[Command] = (stringCI("/buy ") ~> wordIntoBark <~ string("\""),
    whitespaces ~> money).mapN(SimpleCommand.Buy(_, "some", _))
