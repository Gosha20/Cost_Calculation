...
trait PurchaseResource extends Resource {
  ...
  def purchaseRoutes: Route = touchRequiredSession(oneOff, usingHeaders) {
    session =>
      pathPrefix("purchase") {
        (path("add") & post) {
          entity(as[PurchaseCreate]) { purchase =>
            {
              completeWithLocationHeader(resourceId = purchaseService
          ...
          path(IntNumber) { id =>
            get {
              complete(
                purchaseService.getPurchase(id, session.login).unsafeToFuture)
            }
          } ~
          (path("all") & get) {
